...
class Partes_Rover:
    ...
    def __init__(self,textura):
        ...
        self.hitbox=self.textura.get_rect()
        self.hitbox.center=(ANCHO_PC//4,ALTO_PC//8)
    def descripcion(self,texto):
        self.texto=texto

class Boton:
    def __init__(self,texto,fuente,color_texto):
        self.hitbox=pygame.Rect(0,0,180,40)
        self.color_fondo=(0,230,150)
        pygame.draw.rect(pantalla,self.color_fondo,self.hitbox,border_radius=5)
        
        self.fuente_boton=fuente
        self.txt=self.fuente_boton.render(texto,True,color_texto)
        self.txt_rect=self.txt.get_rect(center=self.hitbox.center)
        pantalla.blit(self.txt,self.txt_rect)
...
rueda=Partes_Rover(textura_rueda)
...
camara=Partes_Rover(textura_camara)
...
base=Partes_Rover(textura_base)
        ...
        for event in pygame.event.get():
            ...
            #SELECCION DE LAS PARTEEESSSS
                ...
                if base.hitbox.collidepoint(event.pos):
                    ...
                    texto_display(camara.texto,NEGRO)
                    SELECCIONADOS.append("CÁMARA")
                elif termine.hitbox.collidepoint(event.pos):
                    if len(SELECCIONADOS)>=3:
                        texto_display("Has terminado de construir tu rover!")
                elif len(SELECCIONADOS)<3:
                    texto_display("Renee: Seguro de que has terminado de construir?")
        fisica.step(1/60.0)
