...
def main():
    ...
    # Coordenadas iniciales corregidas (Círculo Azul de la izquierda)
    # Se calculan de forma dinámica al iniciar la exploración
    x_jugador = None   
    y_jugador = None  
    velocidad_jugador = 5
    ...
    while True:
        ...
        # Reducción drástica del tamaño para que entre holgadamente en la habitación sin puerta
        ...
        # Inicialización de posición en el círculo azul una vez que conocemos el tamaño real de pantalla
        ...
        if estado_actual == "exploracion":
            ...
            # --- NUEVA PARAMETRIZACIÓN DEL PASILLO (ZONA ROSA) ---
            Y_MIN_PASILLO = int(H_ACTUAL * 0.44)  # Techo del corredor central
            Y_MAX_PASILLO = int(H_ACTUAL * 0.62)  # Suelo del corredor central
            X_ENTRADA_HAB3 = int(W_ACTUAL * 0.74) # Entrada a la sala de la derecha

            # Verificación de posición respecto a la habitación de la derecha
            if x_jugador < X_ENTRADA_HAB3:
                # Si está en el pasillo común, no puede salirse de la franja rosa central
                if y_jugador < Y_MIN_PASILLO:
                    y_jugador = Y_MIN_PASILLO
                if y_jugador > Y_MAX_PASILLO:
                    y_jugador = Y_MAX_PASILLO
            else:
                # Si entra a la zona de la derecha, se le habilita subir hacia la cabina
                if y_jugador < int(H_ACTUAL * 0.12):
                    y_jugador = int(H_ACTUAL * 0.12)
                if y_jugador > Y_MAX_PASILLO:
                    y_jugador = Y_MAX_PASILLO
                
                # Paredes laterales internas de la habitación de arriba
                if y_jugador < Y_MIN_PASILLO:
            ...
            # Límites estructurales absolutos de la pantalla
            ...
            # --- CORRECCIÓN TRIGGER DEL MINIJUEGO ---
            ...
            if asteroides_destruidos >= max_asteroides_mision:
                ...
                frames_cartel = 0
                estado_actual = "exploracion"
                # Te reubica de forma segura en la zona rosa al salir
        ...
        if estado_actual == "menu":
            ...
            # --- DIBUJO DE ALERTA PEQUEÑA DENTRO DE LA HABITACIÓN ---
            if not mision_completada:
                ...
                # Borde sutil para delimitar la consola en emergencia
